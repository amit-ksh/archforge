...
`design_system` accepts architecture metadata, requirements/constraints, desired capabilities and connections, existing infrastructure, optional provider preference, and optional explicit resolutions. Request-local keys correlate components and connections to canonical IDs generated by the injected ID factory. The tool validates the full strict request and an in-memory domain preview before any write, then composes primitive application commands in dependency order. It returns each executed primitive step, key-to-ID correlations, affected IDs, final validation issues, unresolved decisions, and final revision. It does not silently select technologies or services when evidence is insufficient. Preflight failures report zero writes; runtime failures report the completed prefix, failed step, persisted revision, and recovery guidance. The workflow is sequential and non-atomic: it never claims rollback.
